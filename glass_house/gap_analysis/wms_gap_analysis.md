...
**生成时间**：2026-02-24T09:42:30.337614  
...
```
openapi: 3.0.3
info: { title: WMS Cell API, version: 1.0.0 }
servers: [{ url: /api/v1/wms }]
paths:
  /inventory:
    get:
      parameters: [{ name: X-Tenant-Id, in: header, required: true }, { name: warehouseId, in: query }]
...
2. 运行 `./run.sh verify wms` 或 `./scripts/verify_delivery.sh wms` 通过后置为 production_ready。
