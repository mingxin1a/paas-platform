...
**生成时间**：2026-02-24T09:42:30.301531  
...
**细胞代码**：ems | **领域**：智慧供应链  
**网关前缀**：/api/v1/ems | **注册服务名**：ems-cell | **监控维度**：ems.*

概述：能源管理，能耗数据、计量点、告警/阈值。经网关/注册中心/监控对接；跨细胞仅事件或异步 API。事件域 ems。发布：ems.consumption.recorded, ems.alert.triggered。订阅：mes.work_order.started, mes.equipment.status_changed。
...
```
openapi: 3.0.3
info: { title: EMS Cell API, version: 1.0.0 }
servers: [{ url: /api/v1/ems }]
paths:
  /consumption-records:
    get:
...
2. 运行 `./run.sh verify ems` 或 `./scripts/verify_delivery.sh ems` 通过后置为 production_ready。
